...
from DES import Avalanche_interface
...
class AvalancheApp(QtWidgets.QMainWindow,
    ...
    def __init__(self, parent):
        ...
        self.blocks_in = feist.block64bits(parent.inText)
        self.blocks_out = feist.block64bits(parent.outText)
        self.inText = None
        self.outText = None
    ...
    def process(self, index: int):
        ...
        new_out_text = text
        new_out_text = feist.encode(
            self.parent().mode, self.parent().initialialize_vector,
            new_out_text, 16, self.parent().keys
        )[1]

        new_out_text = feist.block64bits(new_out_text)[0]
